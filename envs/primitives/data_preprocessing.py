...
class OrdinalEncoderPrim(primitive):
    # can handle missing values. Operates on all categorical features.
    def __init__(self, random_state=0):
        super(OrdinalEncoderPrim, self).__init__(name='OrdinalEncoder')
        self.id = 5
        self.hyperparams = []
        self.type = 'data preprocess'
        self.description = "Encode categorical features as an integer array."
        self.hyperparams_run = {'default': True}
        self.cat_cols = None
        self.preprocess = {}
        self.accept_type = 'b'
    ...
    def fit(self, data):
        ...
        for col in self.cat_cols:
            self.preprocess[col] = OrdinalEncoder().fit(x[col].astype(str))
