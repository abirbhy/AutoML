...
@metafeatures.define("DatasetDimensionality")
class DatasetDimensionality(MetaFeature):
    def _calculate(self, X, y, categorical):
        return float(metafeatures["NumberOfInstances"](X, y).value) / \
               float(metafeatures["NumberOfFeatures"](X, y).value)


@metafeatures.define("LogDatasetDimensionality", dependency="DatasetDimensionality")
class LogDatasetDimensionality(MetaFeature):
    def _calculate(self, X, y, categorical):
        return np.log(metafeatures.get_value("DatasetDimensionality"))
...
@metafeatures.define("LandmarkDecisionTreeRegression")
class LandmarkDecisionTreeRegression(MetaFeature):
    def _calculate(self, X, y, categorical):
        import sklearn.tree
        ...
        for train, test in kf.split(X, y):
            ...
            tree = sklearn.tree.DecisionTreeRegressor(random_state=random_state)
            ...
            accuracy += sklearn.metrics.mean_squared_error(predictions, y[test])
        return accuracy / 2
...
class LandmarkDecisionTreeRegression(MetaFeature):
    def _calculate(self, X, y, categorical):
        ...
        for train, test in kf.split(X, y):
            random_state = sklearn.utils.check_random_state(42)
            tree = sklearn.tree.DecisionTreeRegressor(max_depth=1, random_state=random_state)
...
class LandmarkDecisionTreeRegression(MetaFeature):
    def _calculate(self, X, y, categorical):
        def get_regression_scores(X, y, n_folds=4):
            ...
            for regressor_name, regressor in regressors.items():
                scores = cross_validate(
                    regressor, X, y, cv=cv, scoring=scoring)
                landmarking_scores[regressor_name +
                                   '_mse'] = -1.0 * np.nanmean(scores['test_neg_mean_squared_error'])
                landmarking_scores[regressor_name +
                                   '_mae'] = -1.0 * np.nanmean(scores['test_neg_mean_absolute_error'])
                landmarking_scores[regressor_name +
                                   '_r2'] = np.nanmean(scores['test_r2'])
            return landmarking_scores
        return get_regression_scores(X, y, n_folds=4)
