from . metrics import Accuracy, MSE, MAE, R2
from . import Datasets as ds
import numpy as np

metrics = [Accuracy().name, Accuracy(balanced=False).name, MSE().name, MAE().name, R2().name]
...
def load_all_learning_jobs(mode='train', task='Regression', metric=R2()):
